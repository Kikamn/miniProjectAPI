...
def test():
    ...
    #2. UPDATE USER
    randomNameUpdate = f"{faker.name()} Edit"
    randomEmailUpdate = faker.email()
    payload = {
        "name": randomNameUpdate,
        "gender": "female",
    ...
    req = requests.patch(f"{api_user}/{idNewUser}", headers=head, json=payload) # nnti akan jadi https://gorest.co.in/public/v2/users/6998653
    # print(req.json())
    ...
    assert_that(resName).is_equal_to(randomNameUpdate)
    assert_that(resEmail).is_equal_to(randomEmailUpdate)
